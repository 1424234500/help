...
hashmap 数组链表 基于哈希表实现 hashCode()和equals()	可以调优初始容量和负载因子。 key可以为Null，value可以null  key hashcode equal
	hash冲突 导致退化链表	加载因子为0.75 超过扩容2倍(方便索引扩散) 理想状态数组 链表长度0/1
		hashmap并发环形链 多线程同时put扩容  元素都需要被重算rehash重hash transfer移动 并发导致循环引用 在get时死循环
			负载因子 扩容 rehash  
ConcurrentHashMap
	initialCapacity 初始总容量 16
	loadFactor 加载因子 0.75 
	concurrencyLevel 并发级别 16 Segment 个数 不变

	JDK1.7版本 ReentrantLock+Segment+HashEntry+数组链表 分段锁  Segment的大小最多65536个  concurrencyLevel/DEFAULT_CONCURRENCY_LEVEL =16  
 		段Segment继承重入锁ReentrantLock(AQS) 每个Segment越来越大 锁的粒度大 效率 退化hashtable？
		并发加锁 高并发下单 细分多key锁？
	...
	并发size一致性
		3次不加锁 遍历计数 结果一致 则返回
		否则 给每个Segment加上锁遍历计数 返回 
	
	JDK1.8版本 synchronized+CAS+HashEntry++数组链表8/红黑树 二分红黑树查找键  加快检索 实现compareable
		...
		为什么是synchronized
			...
			JDK优化 锁粗化、锁消除、锁自旋	
	并发size 并发CAS计算好 然后你size的时候直接给你
 
	
 
			
LinkedHashMap保存了记录的插入顺序 
Hashtable	synchronized kv均不为null	所有涉及到多线程操作的都加上了 synchronized 关键字 无法锁 null
...
程序计数器 线程独有周期 方法区中的方法字节码由引擎读取下一条指令，它是一个非常小的内存空间
	为什么有这种东西呢 每个线程都是有一个程序计数器的 相当一个指针
...
4.final 作用 初始化 闭包? 生命周期改变 子线程可见域
	java1.8的编译智能 无修改情况下 默认final 如果你重新给局部变量改变了值或引用，那就无法默认为final了
...
ThreadLocal  数据副本 线程切面耗时
	...
	线程池有四种状态，分别为 RUNNING、SHURDOWN、STOP、TERMINATED。
	线程池创建后处于RUNNING状态。
	...
	调用shutdownNow后处于STOP状态，发出interrupted以中止线程的运行。各个线程会抛出InterruptedException异常(前提是线程中运行了sleep、interrupted等方法) 队列中的任务移除并以列表形式返回
		...
		如何实现线程池 所有任务执行完毕后关闭
		CountDownLatch 并发计数器 实现超时等待	CountDownLatch.await(300,TimeUnit.SECONDS);
